...
# read the data
        ...
        # This for loop saves all of the SDS in the dictionary at the top (dependent on file type) to the array
        # (with titles)
        for i in range(0, len(vlist)):
            ...
            if len(sds.shape) == 3:
                ...
                data = sds[:].ravel()
                # The next few lines change fill value/missing value to NaN so that we can multiply valid values by the
                # scale factor, then back to fill values for saving
